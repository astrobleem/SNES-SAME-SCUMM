...
S6 has a historical bounded preflight against an externally supplied Fate of Atlantis
...
branch to the opcode core. `make s6-preflight` verifies the configured archive
identity and presence of its expected distribution notice (it is not a rights
adjudication), 10 rooms, 74 scripts, 27 readable sounds, 20
costumes, three charsets, logical pointer input, and a SAME save-state round trip.

This remains an in-progress S6 gate for a user-supplied external archive. The real boot script now crosses the C8 string workload,
...
The Fate demo template is stored beside it. A separately obtained demo archive
may be extracted without altering its notices into:
