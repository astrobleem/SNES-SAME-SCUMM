...
> The Fate/Monkey source-audio conversion examples below are historical
> engineering records. Their sample/MML inputs were removed during PR #1 asset
> clearance because redistribution terms were not established. Technical
> listener approval is not rights clearance; only the synthetic QTMA/M24R-A
> fixtures are part of the current reproducible content path.
...
Historically, Fate sound 154 was the first end-to-end consumer. Its taken iMUSE jump path is
...
Its patch fingerprint historically resolved to two manually reviewed Monkey
Island organ samples: `mt32_p13_low` below MIDI 58 and `mt32_p13` at 58 and
above. The source checkout did not establish permission to redistribute those
samples; they and the dependent bank manifest have been removed. The old
mapping was a technical arrangement mapping between AdLib identity and an MI
rendition, not evidence of rights or a claim that the MT-32 waveform equals
OPL.
...
The first real graphs historically regenerated Fate sounds 83 and 154 and
Monkey church. Those title-specific graph and sample inputs are removed; this
is not a current reproducible build path.
