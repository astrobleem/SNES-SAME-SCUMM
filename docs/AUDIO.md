...
## Current distributable fixtures

The default build and regression suite use the project-authored M24R-A
conformance composition and a deterministic additive waveform generated under
ignored `build/` output. Three tiny QTMA `.hex` fixtures are also project
authored and are used only by copyright-free parser tests. The root `LICENSE`
dedicates original project material to the public domain. The TAD driver has a
separate zlib license in `audio/TAD-LICENSE.txt`.

Earlier Fate/Monkey audition samples, BRR conversions, and source-derived MML
were removed from the current tree because no applicable redistribution grant
was established. Their historical reports are not a license, and their former
audio-profile targets are guarded until a separately cleared external content
package is available. See [the PR #1 asset provenance record](PR1_ASSET_PROVENANCE.md)
for the path/hash inventory and replacement policy. No original game archive
is part of the ordinary build or test suite.

## Score intent versus rendition

The sections below preserve historical engineering notes. They describe work
performed when the now-removed Fate/Monkey sample assets were present; their
listening verdicts are technical judgments only, not evidence of ownership or
redistribution permission. They must not be read as saying those media remain
in the repository or are cleared for distribution.
...
C42 started the reviewable instrument-bank workflow. It reused manually
reviewed Monkey Island sample files whose redistribution permission was not
established in this repository; those files have since been removed. The
historical work mapped explicit low/main/high zones so the SNES did not have
to pitch one source across an audibly strained range.
