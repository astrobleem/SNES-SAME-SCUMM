...
def main() -> int:
    parser = argparse.ArgumentParser(description="Refuse to build SAME-VDP with the wrong Poppy fork")
    ...
    if REQUIRED_REPOSITORY not in normalized:
        parser.error(f"Poppy origin is {origin!r}; SAME-VDP requires {REQUIRED_REPOSITORY}")
