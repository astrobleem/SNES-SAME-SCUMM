...
> Historical audio status: the Fate/Monkey samples, BRR derivatives, and
> source-derived MML described in the following legacy entries were removed
> during PR #1 asset clearance. The former listener-review records are not
> redistribution permission and no longer describe assets in the current tree.
> See `docs/PR1_ASSET_PROVENANCE.md` for exact hashes and dispositions.
...
verdict was historically preserved in the now-removed round-one listener file.
Round two
...
historically preserved in the now-removed round-five listener file.
