...
M24RA_TAD_TOOLCHAIN_DIR ?= $(CURDIR)/build/toolchain/tad-m24ra
M24RA_AUDIO_DIR ?= $(CURDIR)/build/profile-music/m24ra
M24RA_TAD_DONOR_DIR ?= $(CURDIR)/../terrific-audio-driver
SAME_FATE_DEMO_ARCHIVE ?=
SAME_MONKEY_ARCHIVE ?=

.PHONY: all fixtures generate m24ra-audio archive-audio-unavailable test validate demo package adventure-package \
...
generate: fixtures
	...
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/generate_snes_scumm_variables.py \
		--synthetic --profile examples/profiles/scumm_v5_conformance.json \
		--include runtime/snes/generated/scumm_v5_variables.inc.pasm \
		--manifest build/test-scumm-v5-variables.json
	...
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/generate_snes_video_overlay.py \
		--overlay none --carrier lorom --backend legacy_backdrop \
		--manifest build/test-video-overlay-manifest.json
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/generate_music_catalog.py \
		audio/m24ra/catalog.json examples/resources/music/test_tad_song_enums.inc \
		runtime/snes/generated/music_catalog.inc.pasm
	cp runtime/snes/tad_layout_lint_stub.inc.pasm runtime/snes/generated/tad_layout.inc.pasm
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/generate_snes_cooked_rooms.py \
		--manifest examples/resources/scumm_v5/empty_cooked_rooms.json \
		--output runtime/snes/generated/scumm_v5_rooms.inc.pasm \
		--data-output runtime/snes/generated/scumm_v5_room_data.inc.pasm \
		--binary-dir build/empty-scumm-rooms/segments \
		--report build/empty-scumm-rooms/report.json

m24ra-audio:
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_m24ra_tad_toolchain.py \
		--donor $(M24RA_TAD_DONOR_DIR) --output $(M24RA_TAD_TOOLCHAIN_DIR)
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_m24ra_fixture.py \
		--compiler $(M24RA_TAD_TOOLCHAIN_DIR)/target/release/tad-compiler \
		--output $(M24RA_AUDIO_DIR)
...
validate: generate
	...
	@set -e; for f in examples/targets/*.json; do PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m same.cli target validate "$$f" >/dev/null; done
	@set -e; for f in \
		examples/profiles/agi_v2_conformance.json \
		examples/profiles/scumm_v5_conformance.json \
		examples/profiles/scumm_v5_s2_conformance.json \
		examples/profiles/scumm_v5_s3_conformance.json \
		examples/profiles/scumm_v5_s4_conformance.json \
		examples/profiles/scumm_v5_s5_conformance.json; do \
		PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m same.cli engine validate "$$f" >/dev/null; \
	done
...
# These historical profile builds depended on unlicensed game-derived
# samples/transcriptions that are no longer distributed. The generic music
# engine and copyright-free M24R-A fixture remain buildable; restore a title
# integration only from a separately cleared, user-supplied content package.
archive-audio-unavailable:
	@echo "FATE/Monkey archive-audio integration is not bundled: game-derived media was removed; use the copyright-free M24R-A fixture or a separately cleared external package." >&2
	@exit 2

demo: package adventure-package engine-demo simulate audio
	cd labs/vdp && PYTHONPATH=../../src $(PYTHON) -m same_vdp.cli verify --root .

snes: generate validate m24ra-audio
	SAME_TAD_PREBUILT_DIR=$(M24RA_AUDIO_DIR) \
	SAME_MUSIC_CATALOG=$(CURDIR)/audio/m24ra/catalog.json \
	SAME_TAD_LAYOUT_OUTPUT=runtime/snes/generated/tad_layout.inc.pasm \
	SAME_SNES_ENGINE=demo SAME_BUILD_M24RA=1 tools/build_snes.sh

s5-snes: generate validate m24ra-audio
	SAME_TAD_PREBUILT_DIR=$(M24RA_AUDIO_DIR) \
	SAME_MUSIC_CATALOG=$(CURDIR)/audio/m24ra/catalog.json \
	SAME_TAD_LAYOUT_OUTPUT=runtime/snes/generated/tad_layout.inc.pasm \
	SAME_SNES_ENGINE=scumm_v5 SAME_BUILD_M24RA=1 \
	SAME_SNES_OUTPUT=build/same-scumm-v5.sfc tools/build_snes.sh
...
s6-preflight: test validate
	@test -n "$(SAME_FATE_DEMO_ARCHIVE)" || { echo "Set SAME_FATE_DEMO_ARCHIVE to a user-supplied archive for this optional integration." >&2; exit 2; }
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/validate_scumm_s6_fate_preflight.py \
		--archive "$(SAME_FATE_DEMO_ARCHIVE)"

s6-tad: archive-audio-unavailable s5-snes
	PYTHONPATH=/home/chad/Mesen2/python $(PYTHON) tools/validate_scumm_s6_tad_nexen.py \
		--rom build/same-scumm-v5.sfc

s6-auditions: archive-audio-unavailable s5-snes
	PYTHONPATH=/home/chad/Mesen2/python $(PYTHON) tools/capture_fate_instrument_auditions.py \
		--rom build/same-scumm-v5.sfc

m4: archive-audio-unavailable test validate
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/validate_monkey_v5_music.py \
		--archive "$(SAME_MONKEY_ARCHIVE)" \
		--output build/monkey-v5-music-m4

m5-build: archive-audio-unavailable
...
m6: archive-audio-unavailable

m7-build: archive-audio-unavailable
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_scumm_v5_music_graph.py \
		...
		--archive "$(SAME_FATE_DEMO_ARCHIVE)" --output build/music-m7-fate
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_scumm_v5_music_graph.py \
		--graph examples/resources/music/monkey_v5_build_graph.json \
		--archive "$(SAME_MONKEY_ARCHIVE)" --output build/music-m7-monkey
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_scumm_v5_music_graph.py \
		--graph examples/resources/music/fate_s6_build_graph.json \
		--archive "$(SAME_FATE_DEMO_ARCHIVE)" --output build/music-m7-fate --verify
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_scumm_v5_music_graph.py \
		--graph examples/resources/music/monkey_v5_build_graph.json \
		--archive "$(SAME_MONKEY_ARCHIVE)" --output build/music-m7-monkey --verify
...
m8-build: archive-audio-unavailable
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_scumm_v5_music_graph.py \
		--graph examples/resources/music/fate_s6_build_graph.json \
		--archive "$(SAME_FATE_DEMO_ARCHIVE)" --output build/music-m8-fate
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_scumm_v5_music_graph.py \
		--graph examples/resources/music/fate_s6_build_graph.json \
		--archive "$(SAME_FATE_DEMO_ARCHIVE)" --output build/music-m8-fate --verify
...
m9-build: archive-audio-unavailable
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_profile_music_rom.py \
		...
		--source-archive "$(SAME_FATE_DEMO_ARCHIVE)" \
	...
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_profile_music_rom.py \
		...
		--source-archive "$(SAME_MONKEY_ARCHIVE)" \
...
m10-title-audio-build: archive-audio-unavailable

m10: archive-audio-unavailable test validate m10-build
	PYTHONPATH=/home/chad/Mesen2/python:$(PYTHONPATH) $(PYTHON) \
		tools/validate_scumm_s6_tad_nexen.py \
...
m19-build: archive-audio-unavailable
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_profile_music_rom.py \
		--profile examples/profiles/templates/monkey1_ultimate_talkie.json \
		--source-archive "$(SAME_MONKEY_ARCHIVE)" \
...
m20-build: archive-audio-unavailable
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_profile_music_rom.py \
		--profile examples/profiles/templates/monkey1_ultimate_talkie.json \
		--source-archive "$(SAME_MONKEY_ARCHIVE)" \
...
m21-build: archive-audio-unavailable
...
m22-build: archive-audio-unavailable
...
m23a-build: archive-audio-unavailable
	...
	SAME_FATE_DEMO_ARCHIVE="$(SAME_FATE_DEMO_ARCHIVE)" tools/build_snes.sh
...
m23b-build: archive-audio-unavailable
	SAME_TAD_PREBUILT_DIR=$(CURDIR)/build/profile-music/indy4-fate-demo-m22 \
	SAME_MUSIC_CATALOG=$(CURDIR)/build/profile-music/indy4-fate-demo-m22/catalog.json \
	...
	SAME_SCUMM_SAVE_LOGICAL_ID=80 SAME_FATE_DEMO_ARCHIVE="$(SAME_FATE_DEMO_ARCHIVE)" \
	tools/build_snes.sh
	SAME_TAD_PREBUILT_DIR=$(CURDIR)/build/profile-music/indy4-fate-demo-m22 \
	SAME_MUSIC_CATALOG=$(CURDIR)/build/profile-music/indy4-fate-demo-m22/catalog.json \
...
m23c-build: archive-audio-unavailable
	SAME_TAD_PREBUILT_DIR=$(CURDIR)/build/profile-music/indy4-fate-demo-m22 \
	SAME_MUSIC_CATALOG=$(CURDIR)/build/profile-music/indy4-fate-demo-m22/catalog.json \
	...
	SAME_FATE_DEMO_ARCHIVE="$(SAME_FATE_DEMO_ARCHIVE)" tools/build_snes.sh
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) tools/cook_scumm_v5_rooms.py --archive "$(SAME_FATE_DEMO_ARCHIVE)" \
...
m24rb-build: archive-audio-unavailable
	PYTHONPATH=$(PYTHONPATH):tools $(PYTHON) tools/build_fate_m24rb_composite.py \
		--archive "$(SAME_FATE_DEMO_ARCHIVE)" --output build/m24rb-content
...
m25a-startobject-replacement: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=startobject-replacement \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/startobject-replacement/m25a-startobject-replacement.sfc \
	tools/build_snes.sh
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
...
m25a-startobject-control: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=startobject-nested \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/startobject-nested/m25a-startobject-nested.sfc \
	tools/build_snes.sh
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_start_object_nexen.py \
		--case nested \
...
m25a-startobject-normal: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=startobject \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/startobject/m25a-startobject.sfc \
	tools/build_snes.sh
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_start_object_nexen.py \
		--case normal \
...
m25a-startscript-replacement: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=startscript-replacement \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/startscript-replacement/m25a-startscript-replacement.sfc \
	tools/build_snes.sh
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_startscript_replacement_nexen.py \
		--rom build/m25a-validator/startscript-replacement/m25a-startscript-replacement.sfc \
...
m25a-global-room-continuation: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=global-room-continuation \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/global-room-continuation/m25a-global-room-continuation.sfc \
	tools/build_snes.sh
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_global_room_continuation_nexen.py \
		--rom build/m25a-validator/global-room-continuation/m25a-global-room-continuation.sfc \
...
m25a-local-room-continuation: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=local-room-continuation \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/local-room-continuation/m25a-local-room-continuation.sfc \
	tools/build_snes.sh
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_local_room_continuation_nexen.py \
		--rom build/m25a-validator/local-room-continuation/m25a-local-room-continuation.sfc \
...
m25a-actor-position-errors: m25a-actor-position-errors-build
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_actor_position_errors_nexen.py \
		--truncated-result-rom build/m25a-validator/actor-position-result-truncated/m25a-actor-position-result-truncated.sfc \
		--invalid-result-rom build/m25a-validator/actor-position-result-invalid/m25a-actor-position-result-invalid.sfc \
...
m25a-actor-position-success-build: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=actor-position-success \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/actor-position-success/m25a-actor-position-success.sfc \
	tools/build_snes.sh

m25a-actor-position-success: m25a-actor-position-success-build
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_actor_position_success_nexen.py \
		--rom build/m25a-validator/actor-position-success/m25a-actor-position-success.sfc \
...
m25a-null-room-lifecycle-build: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_M23A=1 SAME_BUILD_SCUMM_M23B=1 SAME_BUILD_SCUMM_M23C=1 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=1 SAME_SCUMM_SCENARIO_START_ROOM=49 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 SAME_M25A_VALIDATOR_CASE=null-room-lifecycle \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/null-room-lifecycle/m25a-null-room-lifecycle.sfc \
	tools/build_snes.sh

m25a-null-room-lifecycle: m25a-null-room-lifecycle-build
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_null_room_lifecycle_nexen.py \
		--rom build/m25a-validator/null-room-lifecycle/m25a-null-room-lifecycle.sfc \
...
m25a-pending-room-request-near-build: m25a-validator-audio
	...
	SAME_BUILD_SCUMM_ROOM_SERVICE_FAR=0 \
	SAME_BUILD_SCUMM_SCENARIO_FIXTURE=0 \
	SAME_BUILD_SCUMM_M25A_VALIDATOR=1 \
	SAME_BUILD_SCUMM_M25A_PENDING_ONLY=1 \
	SAME_M25A_VALIDATOR_CASE=pending-room-request \
	SAME_SNES_PROFILE=$(CURDIR)/examples/profiles/m25a_nested_conformance.json \
	SAME_SNES_OUTPUT=$(CURDIR)/build/m25a-validator/pending-near/m25a-pending-near.sfc \
	tools/build_snes.sh

m25a-pending-room-request: m25a-pending-room-request-build m25a-pending-room-request-near-build
	@for route in far near; do \
		if [ "$$route" = far ]; then rom=build/m25a-validator/pending-far/m25a-pending-far.sfc; \
		else rom=build/m25a-validator/pending-near/m25a-pending-near.sfc; fi; \
		for phase in 4 5; do \
...
m25a-lookup-missing: m25a-lookup-missing-build
	PYTHONPATH=/home/chad/Mesen2/python:src:tools $(PYTHON) \
		tools/validate_scumm_room_local_lookup_nexen.py \
		--missing-rom build/m25a-validator/lookup-missing/m25a-lookup-missing.sfc \
		--missing-global-rom build/m25a-validator/global-lookup-missing/m25a-global-lookup-missing.sfc \
		--missing-only \
		--output build/m25a-validator/unmapped-starts-evidence.json
