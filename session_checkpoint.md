...
## 2026-09-11 — standalone ENCD nested-return matrix

The standalone target lane assembles with Poppy DLL SHA-256
`34514923ea8dc79a4664fa327f583cee8e8daa64e3be47518ae22ba5a2c7608e` from
source line `8ee859b33bad94e3a01e9c78026803e482292801`. The frozen Nexen
...
`build/current-room42-final-run4/report.json`; room49 replay is recorded at
`build/current-room49-authenticated-replay3/report.json`. The standalone
enabled/disabled runtime acceptance remains represented by ROMs
`629fecf3d4113ab306dc374fd345acb665211cfb01ed3fa881eec5077756fc62` and
...
return-mode handling: bare ENCD (`STOP`) passed trace `9 -> 10` and phase
`2 -> 0`; nested `startScript(200) -> STOP` failed before the fix while
preserving slot zero at program 208/PC 3 and leaving child program 210
stopped; the authored VerbOps case shared that frontier. The nested runner
now saves/restores parent return mode and clears transient nested depth around
child execution. Fixed target ROM
`f74f4d577a34ce80cc01397951ace1902900b17aa098d2dd16f7fa65256f1bcf` boots
room 1 to phase 0 with lifecycle `1,2,4,5,6,7,8,9,10`, controller-ready 1,
and error 0. Authored script 200 populates C17 verb 3 as `Push`.

## 2026-09-11 — standalone controller runtime replay

Using the frozen Nexen executable (SHA-256
`17d243c404b8ef32bbb1754a5b026584f2ae24cb047f54b9f250a6f4b721650a`) and
historical Poppy DLL SHA-256
`34514923ea8dc79a4664fa327f583cee8e8daa64e3be47518ae22ba5a2c7608e`, the
enabled standalone replay passed room install, phase 0/readiness/error 0,
natural cursor movement, object 7 CDHD hit, authored verb 3/C17 `Push`,
sentence submission, generated launcher and OBCD child retirement, and
canonical ACTION_PENDING re-entry. The authored action set fixture variable 5
to 1 through normal SetVarRange before STOP; completion did not depend on that
effect. A post-action D-pad edge moved the cursor from 36 to 38.

The first attempted disabled companion was invalid because build_snes.sh forced
`--scumm-controller` for every conformance build. That selector was corrected;
the real disabled build has a distinct ROM SHA
`350abfa2d76fd0c5291661197eef9490d6f11297bde862a7436e99ca809274ff` and boots
room 1 to phase 0/error 0 without controller cursor/action side effects.

The enabled final replay ROM SHA is
`629fecf3d4113ab306dc374fd345acb665211cfb01ed3fa881eec5077756fc62`.
The legacy-backdrop standalone capture is one-color, so no native text-pixel
claim is made for this personality; semantic HUD/runtime payload behavior was
observed. Fate room42/room49 reruns remain pending because their exact accepted
ROM artifacts are not present in this worktree.

## 2026-09-11 — current-branch Fate rebuild gate

The accepted Fate inputs were rebuilt from `/home/chad/ATLANTIS.zip` with
frozen Nexen SHA-256 `17d243c404b8ef32bbb1754a5b026584f2ae24cb047f54b9f250a6f4b721650a`
and historical Poppy DLL SHA-256
`34514923ea8dc79a4664fa327f583cee8e8daa64e3be47518ae22ba5a2c7608e`.
Current room42 is `build/current-room42-final.sfc`, SHA
`c17edca9ee428d734b8835c55e75bc381c74f8ec33c1d45a7de04c8d59a04daa`; cold
replay `build/current-room42-final-run3/report.json` passes the accepted
Open, state-neutral, object492 Push, post-action, and error-zero checks.
Focused SCUMM tests are 257/257; Poppy guard, Python compilation, and diff
check pass. The broad suite is 547 pass, 1 skipped, 1 fail: the previously
adjudicated Phase6K generated byte-oracle drift.

The current room49 rebuilds prove boot and natural object592/verb9 selection,
but the replay helper did not submit the second A while HUD retry status was
busy. The recorded pre-M25A configuration also failed to reproduce the
accepted controller-ready root. This is not accepted as a production
failure; current-branch room49 replay and publication remain blocked on
recovering the exact accepted scenario-root invocation. No production source
was changed for this observation mismatch.

The follow-up completed-frame HUD trace is more specific: the M25A `salvage`
manifest installs room49/object592 but does not execute the authenticated
global script18 VerbOps producer. C17 verb9 consequently contains stale/non-
text bytes; BuildHudText cannot publish a valid HUD, leaving
`TALK_VISUAL_STATUS` and `HUD_DIRTY` set after the initial generation commits.
The controller correctly rejects the second A. An attempted
`--prepend-global-script` addition to the cooked-room generator was rejected
by that tool's interface and reverted; no production or build-script change
was retained. The next step is to recover the established cooker prepend root.

## 2026-09-11 — Authenticated room49 C17 root replay

Recovered the established fixture-boundary mechanism from the room-boundaries
worktree: the ordinary Phase6LA1D cooker path uses
`tools/cook_scumm_v5_rooms.py --prepend-global-script 144 18`. The M25A salvage
generator path does not execute that authored producer and was not modified.

The current-source rebuild used the pinned Poppy DLL
`34514923ea8dc79a4664fa327f583cee8e8daa64e3be47518ae22ba5a2c7608e`, FULL
`/home/chad/ATLANTIS.zip`, the Phase6LA1D room49 profile, and 4 MiB SA-1
configuration. Assembly, Poppy lint, finalization, and ROM audit passed.

ROM: `build/current-room49-authenticated.sfc`, SHA-256
`a7b96d532c33e122f571ae4284ae89c2e318d93d70cb430a8a57ba004547bb4d`.

Cold room49 replay artifact:
`build/current-room49-authenticated-replay3/report.json`. It reaches room49
phase 0/error 0, moves the real cursor, selects live CDHD object 592, resolves
authored verb 9 with runtime C17 text `Look at`, composes the source/runtime
HUD `Look at salvage boat`, reaches the committed overlay boundary, submits
the second A through the ordinary sentence API, enters and releases
`ACTION_PENDING`, and returns to usable controller state with error 0. No raw
controller, object, verb, C17, or script state was written by the replay.

The current room42 controller replay was rerun with frozen Nexen and passed:
Open/state-changing action, canonical ACTION_PENDING release, state-neutral
action, object492 Push, and post-action input, all with error 0. Focused SCUMM
tests remain 257/257; the repository suite remains 547 pass, 1 skip, and the
