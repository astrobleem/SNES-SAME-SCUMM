...
object or action; Y changes the authored verb. Sentence completion is owned
by the normal SCUMM lifecycle, not by an object-state policy. No mailbox or
game-state writes are part of the manual route.
"""
...
CONTROLLER_MODE_HOOK = None
LOGICAL_FRAME_HOOK = None
LOGICAL_FRAME_ADDRESS = None
CONTROLLER_TRACE_HOOKS = {}
SEMANTIC_MOVING_HOOK = None
HUD_TRACE_HOOKS = {}
EMULATOR_PROVENANCE = None
...
def emulator_provenance_for(executable: Path) -> dict[str, object]:
    """Record the actual runner and, for wrappers, the executed binary."""
    path = executable.resolve()
    result: dict[str, object] = {
        "requested_path": str(executable),
        "resolved_path": str(path),
        "requested_sha256": hashlib.sha256(executable.read_bytes()).hexdigest(),
        "requested_mtime": executable.stat().st_mtime,
    }
    text = executable.read_text(errors="replace") if executable.suffix in (".sh", ".bash") else ""
    result["wrapper_sha256"] = result["requested_sha256"] if text else None
    result["wrapper_text"] = text if text else None
    target = None
    for line in text.splitlines():
        if "exec dotnet " in line:
            target = Path(line.split("exec dotnet ", 1)[1].split()[0])
            break
    if target is not None and target.is_file():
        result.update({
            "executed_binary_path": str(target.resolve()),
            "executed_binary_sha256": hashlib.sha256(target.read_bytes()).hexdigest(),
            "executed_binary_mtime": target.stat().st_mtime,
        })
    else:
        result.update({"executed_binary_path": str(path),
                       "executed_binary_sha256": result["requested_sha256"]})
    return result


REPLAY_CONTRACT_ENV = (
    "SAME_FATE_DEMO_ARCHIVE",
    "SAME_M25A_VALIDATOR_CASE",
    "SAME_BUILD_SCUMM_SCENARIO_FIXTURE",
    "SAME_SCUMM_SCENARIO_START_ROOM",
    "SAME_BUILD_SCUMM_ROOM_VISUAL_ROOMS",
    "SAME_BUILD_SCUMM_CONTROLLER_FIXTURE",
    "SAME_SCUMM_SCENARIO_CLASS_OVERLAY",
    "SAME_SCUMM_SCENARIO_SOURCE_ACTOR_STATE",
)


def replay_contract_for(rom: Path) -> dict[str, object] | None:
    """Return the semantic/configuration identity for a controller replay.

    ROM hashes identify artifacts; this contract identifies whether two replay
    runs exercised the same authored scenario.  It intentionally permits
    production-source differences while making omitted start-room/visual-room
    flags visible and comparable.
    """
    sidecar = rom.with_suffix(".build_identity.json")
    if not sidecar.is_file():
        return None
    identity = json.loads(sidecar.read_text())
    env = identity.get("explicit_environment", {})
    contract = {key: env.get(key, "") for key in REPLAY_CONTRACT_ENV}
    manifest_path = env.get("SAME_SCUMM_ROOM_VISUAL_MANIFEST", "")
    manifest_sha = None
    room42_encd = None
    if manifest_path and Path(manifest_path).is_file():
        ...
        for record in manifest.get("records", manifest if isinstance(manifest, list) else []):
            if str(record.get("room", "")) != "42":
                continue
            script_records = record.get("scripts", [record])
            for script in script_records:
                if str(script.get("kind", script.get("type", ""))).upper() != "ENCD":
                    continue
                payload = script.get("bytes", script.get("data"))
                if isinstance(payload, list):
                    raw = bytes(payload)
                    room42_encd = {"length": len(raw), "sha256": hashlib.sha256(raw).hexdigest()}
                    ...
                    try:
                        ...
                        pass
                if room42_encd is None and script.get("chunk_length"):
                    room42_encd = {"length": int(script["chunk_length"]),
                                    "sha256": script.get("sha256")}
                break
    result = {
        "rom_sha256": hashlib.sha256(rom.read_bytes()).hexdigest(),
        "poppy_dll_sha256": identity.get("poppy_sha256"),
        "poppy_source": env.get("POPPY_ROOT"),
        "corpus": identity.get("corpus"),
        "environment": contract,
        "manifest_sha256": manifest_sha,
        "room42_encd": room42_encd,
    }
    result["scenario_contract_sha256"] = hashlib.sha256(
        json.dumps({"environment": contract, "room42_encd": room42_encd}, sort_keys=True).encode()
    ).hexdigest()
    return result
...
def move_cursor_to_source_object(session, object_id: int, limit: int = 128) -> tuple[dict[str, int], int, int]:
    ...
    for _ in range(limit):
        ...
        # Steer by the source rectangle, not by a single canonical point.
        # A valid CDHD rectangle can extend beyond the visible cursor range;
        # the nearest visible point inside the rectangle is still a valid
        # source hit and avoids an unreachable-point loop.
        if room_x < record["x"]:
            button = session.BTN_RIGHT
        elif room_x >= record["x"] + record["width"]:
            button = session.BTN_LEFT
        elif room_y < record["y"]:
...
def read_scene(session) -> dict[str, int]:
    ...
    verb_id = session.read_memory("snesMemory", CONTROLLER + 0x05, 1)[0]
    verb_offset = verb_id * 0x60
    verb_record = session.read_memory("snesMemory", 0x7F6F20 + verb_offset, 0x60)
    object_id = session.read_memory("snesMemory", CONTROLLER + 0x06, 2)
    object_id = u16(object_id)
    object_name_length = session.read_memory("snesMemory", 0x7E8800 + object_id, 1)[0]
    object_name = list(session.read_memory("snesMemory", 0x7E9000 + object_id * 0x20, 0x20))
    return {
        ...
        "talk_raw": list(session.read_memory("snesMemory", 0x7E7A30, 32)),
            "hud_runtime": {
            "verb_id": verb_id,
            "verb_name_length": verb_record[0x18],
            "verb_name": list(verb_record[0x20:0x60]),
            "object_id": object_id,
            "object_name_length": object_name_length,
                "object_name": object_name,
            },
            "hud_text_service": {
                "valid": session.read_memory("snesMemory", 0x7E5FC5, 1)[0],
                "length": session.read_memory("snesMemory", 0x7E5FC4, 1)[0],
                "bytes": list(session.read_memory("snesMemory", 0x7E5FA4, 32)),
            },
        "hud_text_service": {
            "valid": session.read_memory("snesMemory", 0x7E5FC5, 1)[0],
            "length": session.read_memory("snesMemory", 0x7E5FC4, 1)[0],
        ...
        "verb_api": {
            ...
            "id": u16(session.read_memory("snesMemory", 0x7E7EC1, 2)),
            "result": u16(session.read_memory("snesMemory", 0x7E7EC3, 2)),
            "after": u16(session.read_memory("snesMemory", 0x7E5F9A, 2)),
            "limit": u16(session.read_memory("snesMemory", 0x7E5F98, 2)),
        },
...
def advance_logical_safe(session, max_frames: int = 64) -> None:
    """Advance to a completed Same_Main_Loop boundary.

    This is the validator's authoritative observation fence.  The hook is
    placed at the existing post-Same_Frame_Run main-loop re-entry; no
    production timing is changed.  Falling back to an arbitrary frame batch
    would allow semantic predicates to sample the middle of a logical frame,
    so an uninstalled fence is a validator error rather than a safe mode.
    """
    global LOGICAL_FRAME_HOOK
    if LOGICAL_FRAME_HOOK is None:
        raise RuntimeError("logical-frame observation fence is not installed")
    before = u16(session.read_memory("snesMemory", 0x7E2308, 2))
    before_frame = u16(session.read_memory("snesMemory", 0x7E2210, 2))
    for attempt in range(4):
        session.resume()
        result = session.run_until(max_frames=max_frames,
                                   hook_handle=LOGICAL_FRAME_HOOK)
        session.pause()
        after = u16(session.read_memory("snesMemory", 0x7E2308, 2))
        after_frame = u16(session.read_memory("snesMemory", 0x7E2210, 2))
        if result.get("timedOut"):
            raise RuntimeError(f"logical-frame fence timed out: {result}")
        # Nexen can report a hook immediately when the CPU was already paused
        # at the bank-0 loop entry.  That is not a completed logical frame.
        # Advance one ordinary emulator frame with the hook removed, then
        # reinstall it and require the logical-frame counter to move.
        if after != before or after_frame != before_frame:
            return
        session.remove_hook(LOGICAL_FRAME_HOOK)
        LOGICAL_FRAME_HOOK = None
        session.resume()
        physical = session.run_frames(1)
        session.pause()
        if physical.get("timedOut") or physical.get("framesAdvanced") != 1:
            raise RuntimeError(f"stale logical-frame hook could not be cleared: {physical}")
        LOGICAL_FRAME_HOOK = session.add_exec_hook(LOGICAL_FRAME_ADDRESS)
        before = u16(session.read_memory("snesMemory", 0x7E2308, 2))
        before_frame = u16(session.read_memory("snesMemory", 0x7E2210, 2))
    raise RuntimeError(
        "logical-frame hook fired without a completed logical-frame counter advance: "
        f"before={before}/{before_frame} after={after}/{after_frame} "
        f"cpu={session.get_cpu_state('Snes')}"
    )


def advance_logical_count(session, count: int) -> None:
    """Advance semantic observations by completed logical frames only."""
    for _ in range(count):
        advance_logical_safe(session)


def run_input_window(session, mask: int, frames: int = 2) -> None:
    """Run a short real input edge without an execution-hook interruption."""
    global LOGICAL_FRAME_HOOK
    saved = LOGICAL_FRAME_HOOK
    if saved is not None:
        session.remove_hook(saved)
        LOGICAL_FRAME_HOOK = None
    try:
        session.set_input(mask, frames)
    finally:
        if saved is not None:
            LOGICAL_FRAME_HOOK = session.add_exec_hook(LOGICAL_FRAME_ADDRESS)


def set_input_level(session, mask: int) -> None:
    """Set the Nexen controller level without advancing emulation.

    The auto-advance ``frames`` form clears the override after its run.  That
    is useful for coarse cursor motion, but it cannot express a precise
    HELD/PREVIOUS edge at a completed logical-frame boundary.  Nexen's
    hold=true form is the level primitive; the caller fences the resulting
    frame explicitly.
    """
    session.tool("set_input", {
        "port": 0,
        "buttons": mask,
        "hold": True,
    })
...
def run_held_level(session, mask: int, frames: int) -> None:
    """Sample a held controller level for raw frames, without a hook stop."""
    global LOGICAL_FRAME_HOOK
    saved = LOGICAL_FRAME_HOOK
    if saved is not None:
        session.remove_hook(saved)
        LOGICAL_FRAME_HOOK = None
    try:
        set_input_level(session, mask)
        result = session.run_frames(frames)
        if result.get("timedOut") or result.get("framesAdvanced") != frames:
            raise RuntimeError(f"unsafe input-level run: {result}")
    finally:
        if saved is not None:
            LOGICAL_FRAME_HOOK = session.add_exec_hook(LOGICAL_FRAME_ADDRESS)
...
def tap(session, button: int) -> None:
    ...
    run_input_window(session, button, min(INPUT_HOLD_FRAMES, 4))
    # Keep the release level across a complete auto-joypad seam.  A single
    # release frame can leave the preceding directional bit latched when the
    # next logical frame is fenced, causing a later A edge to be observed as
    # the stale direction instead of a new controller action.
    run_input_window(session, 0, 2)


def action_tap(session, button: int) -> None:
    """Deliver one real action edge, not a multi-frame button hold."""
    # SAME derives PRESSED from ~PREVIOUS & HELD. A held action therefore
    # creates only one edge, on the first sampled frame; extra held frames do
    # not retry a controller that was not eligible on that edge. Establish a
    # completed released frame, assert the action for exactly one emulated
    # frame, then release it for exactly one emulated frame.
    global RESTORE_DEBUG_HOOK, CONTROLLER_MODE_HOOK, CONTROLLER_TRACE_HOOKS
    global LOGICAL_FRAME_HOOK, SEMANTIC_MOVING_HOOK
    removed_restore = RESTORE_DEBUG_HOOK
    removed_mode = CONTROLLER_MODE_HOOK
    removed_trace = dict(CONTROLLER_TRACE_HOOKS)
    removed_semantic = SEMANTIC_MOVING_HOOK
    for handle in [removed_restore, removed_mode, removed_semantic, *removed_trace.keys()]:
    ...
    try:
        baseline = None
        for _ in range(4):
            run_held_level(session, 0, 2)
            advance_logical_safe(session)
            baseline = session.read_memory("snesMemory", 0x7E2200, 6)
            if not any(baseline):
                break
        require(baseline is not None and not any(baseline),
                f"action edge baseline was not released: {list(baseline or [])}")
        native_action_bits = {session.BTN_A: 0x0080, session.BTN_B: 0x0040,
                              session.BTN_Y: 0x4000}
        native_button = native_action_bits.get(button, button)
        press_byte_address = 0x7E2204 + (1 if native_button > 0x00FF else 0)
        press_byte_value = (native_button >> 8) if native_button > 0x00FF else native_button
        press_hook = session.add_write_hook(press_byte_address,
                                            match_value=press_byte_value,
                                            match_value_mask=press_byte_value)
        set_input_level(session, button)
        saved_logical = LOGICAL_FRAME_HOOK
        if saved_logical is not None:
            session.remove_hook(saved_logical)
            LOGICAL_FRAME_HOOK = None
        session.resume()
        press_result = session.run_until(max_frames=8, hook_handle=press_hook)
        session.pause()
        session.remove_hook(press_hook)
        session.resume()
        post_press = session.run_frames(1)
        session.pause()
        require(not post_press.get("timedOut") and post_press.get("framesAdvanced") == 1,
                f"unsafe post-press frame: {post_press}")
        if saved_logical is not None:
            LOGICAL_FRAME_HOOK = session.add_exec_hook(LOGICAL_FRAME_ADDRESS)
        press = session.read_memory("snesMemory", 0x7E2200, 6)
        print(f"action edge press: {list(press)} result={press_result}", flush=True)
        set_input_level(session, 0)
        if LOGICAL_FRAME_HOOK is not None:
            session.remove_hook(LOGICAL_FRAME_HOOK)
            LOGICAL_FRAME_HOOK = None
        session.resume()
        release_frame = session.run_frames(1)
        session.pause()
        require(not release_frame.get("timedOut") and release_frame.get("framesAdvanced") == 1,
                f"unsafe action release frame: {release_frame}")
        if saved_logical is not None:
            LOGICAL_FRAME_HOOK = session.add_exec_hook(LOGICAL_FRAME_ADDRESS)
    finally:
        if removed_restore is not None:
            RESTORE_DEBUG_HOOK = session.add_exec_hook(0x14863B)
        if removed_mode is not None:
            CONTROLLER_MODE_HOOK = session.add_write_hook(0x7E5FE0)
        for address, label in ((0x9968, "action_pending"), (0x99A9, "refresh_selection"),
                               (0x983F, "cursor_a_hit_test"), (0x9882, "cursor_hit_ok"),
                               (0x98BD, "cursor_verb_ok")):
            if any(label == old_label for old_label in removed_trace.values()):
                handle = session.add_exec_hook(address)
                CONTROLLER_TRACE_HOOKS[handle] = label
        if removed_semantic is not None:
            SEMANTIC_MOVING_HOOK = session.add_write_hook(0x7E5E46,
                                                          match_value=1,
                                                          match_value_mask=0xFF)


def _forensic_snapshot(session, kind: str, *, hook: dict | None = None,
                       mode_before: int | None = None,
                       mode_value: int | None = None,
                       writer: str | None = None) -> dict:
    """Capture one paused hook boundary as a compact forensic record.

    The debugger is paused at the memory writer, so CPU registers and the
    selected interaction fields are read as one observation.  Notifications
    that were queued while another selected hook was being awaited are
    reconciled separately by cycle order; they never replace this paused
    snapshot for the hook that actually stopped the run.
    """
    ...
    object490_records = [record for record in records
                         if record[0] | (record[1] << 8) == 490]
    hook = hook or {}
    return {
        "frame": int(hook.get("frame", u16(session.read_memory("snesMemory", 0x7E2210, 2)))),
        "cycle": int(hook.get("cycleCount", cpu.get("cycleCount", 0))),
        "pc": int(cpu.get("pc", 0)),
        "kind": kind,
        "a": int(cpu.get("a", 0)), "x": int(cpu.get("x", 0)),
        "y": int(cpu.get("y", 0)), "p": int(cpu.get("ps", 0)),
        "sp": int(cpu.get("sp", 0)),
        "controller_mode": ctl[0],
        "input_pressed": u16(session.read_memory("snesMemory", 0x7E2204, 2)),
        "cursor_x": u16(session.read_memory("snesMemory", 0x7E5FE1, 2)),
        "cursor_y": u16(session.read_memory("snesMemory", 0x7E5FE3, 2)),
        "interaction_object": u16(interaction, 8),
        "verb_result": u16(session.read_memory("snesMemory", 0x7E7EC3, 2)),
        "controller_object": u16(ctl, 6),
        "controller_verb": ctl[5],
        "active_object_count": record_count,
        "object490_records": object490_records,
        "active_room": session.read_memory("snesMemory", 0x7FF2BF, 1)[0],
        "object_name_length": session.read_memory("snesMemory", 0x7E8800 + u16(ctl[6:8]), 1)[0],
        "c17_verb3": {
            "present": c17[0], "mode": c17[1], "type": c17[6],
            "name_length": c17[0x18],
        },
        "mode_before": mode_before,
        "mode_value": mode_value,
        "writer": writer,
        "trace_tail": session.trace_log(cpu_type="Snes", count=12),
    }


def _forensic_notifications(session, handles: dict[int, str]) -> list[dict]:
    """Drain hook notifications without treating stale final DIAG as truth."""
    result = []
    for note in session.drain_notifications(timeout=0.0):
...
def _forensic_step_past_write(session, fired_handle: int) -> None:
    """Execute one fired long-store without dropping the other forensic hooks."""
    cpu = session.get_cpu_state("Snes")
    pc = int(cpu["pc"])
    # All four DIAG and controller-mode writes in this corridor are STA.l:
    # opcode + 24-bit address, four bytes total.  Remove only the fired hook,
    # stop at the following instruction, then restore the fired hook.
    session.remove_hook(fired_handle)
    bus_pc = ((int(cpu.get("k", 0)) & 0xff) << 16) | (pc & 0xffff)
    next_hook = session.add_exec_hook(bus_pc + 4)
    session.resume()
    result = session.run_until(max_frames=2, hook_handle=next_hook)
    session.pause()
    session.remove_hook(next_hook)
    if result.get("reason") not in ("hookFired", "hook"):
        raise RuntimeError(
            f"forensic step did not cross writer at {bus_pc:#x}: {result}; cpu={cpu}")


def _capture_hud_builder_ledger(session, *, max_events: int = 96) -> dict:
    """Capture every builder write without changing production execution.

    The debugger pauses on any individual hook.  run_until(hook_handles=...)
    is therefore required: waiting on only the index hook would leave the CPU
    paused on an earlier text-buffer or scratch write and lose the ledger.
    """
    ledger: list[dict] = []
    handles: dict[int, str] = {}
    try:
        # The source-label exec hook is bank-ambiguous in the deployed
        # debugger.  Fence the first object-name read instead; it is the
        # first unambiguous point in the object loop and precedes its store.
        # Arm the source watch together with the builder hooks.  Advancing a
        # preliminary eight-frame read-watch window can consume the first
        # object iteration before the ledger is active, which defeats the
        # handoff's purpose.
        length_read_handle = session.add_read_hook(0x7E89EA)
        handles[length_read_handle] = "object_name_length_read"
        source_read_handle = session.add_read_hook(0x7ECD40, 0x7ECD5F)
        handles[source_read_handle] = "object_source_read"
        # The rebuilt debugger can stop on any hook, so keep the complete
        # write set armed.  This is the instruction-atomic path; the older
        # deployed Nexen fallback is retained in the earlier artifacts.
        watched = {
            (0x7E5F9E, 0x7E5F9F): "dst_index",
            (0x41F6AA, 0x41F6AB): "builder_temp_scratch",
            (0x41F6AC, 0x41F6AD): "remaining_scratch",
            (0x7E7A28, 0x7E7A28): "talk_raw_length",
            (0x7E7AC3, 0x7E7AC3): "talk_segment_length",
            (0x7E5FA4, 0x7E5FC3): "controller_text_buffer",
            (0x7E7A30, 0x7E7A4F): "talk_raw",
        }
        # The generated map for the current ROM places the object loop at
        # $A354.  Keep this execution hook in the same any-hook set as the
        # write watches: it lets the ledger distinguish the builder's object
        # stores from the separate 32-byte buffer clear routine.
        # Capture the whole assembled builder corridor on the deployed
        # runtime.  The single label hook is useful when the map is exact,
        # but the range also catches the actual source-load/store instruction
        # if a generated label moved within the bank.
        # The controller include is in LoROM bank 9.  Use the full 24-bit
        # bus address; a 16-bit hook at $A274 observes an unrelated routine
        # in another bank with the same CPU offset.
        object_loop_handle = session.add_exec_hook(0x09A274, 0x09A38A)
        handles[object_loop_handle] = "builder_exec"
        # Exact first object source-load and destination-store PCs from the
        # assembled bank-9 listing.  These remain separate labels so the
        # ledger can distinguish a zero-valued store (which a write watch may
        # suppress when the destination already contains zero).
        for (start, end), label in watched.items():
            handles[session.add_write_hook(start, end)] = label
        for _ in range(max_events):
            result = session.run_until(max_frames=8, hook_handles=list(handles))
            notes = session.drain_notifications(timeout=0.0)
            hook_notes = [n.get("params", {}) for n in notes
                          if n.get("method") == "notifications/mesen/hookFired"
                          and n.get("params", {}).get("handle") in handles]
            if not hook_notes:
                return {
                    "termination": "timeout_or_no_hook",
                    "run": result,
                    "events": ledger,
                    "state": {
                        "cpu": session.get_cpu_state("Snes"),
                        "dst_index": u16(session.read_memory("snesMemory", 0x7E5F9E, 2)),
                        "remaining": u16(session.read_memory("snesMemory", 0x41F6AA, 2)),
                        "controller_buffer": list(session.read_memory("snesMemory", 0x7E5FA4, 32)),
                        "talk_raw": list(session.read_memory("snesMemory", 0x7E7A30, 32)),
                    },
                }
            # The range target normally yields one store per run.  Preserve
            # every notification in case the debugger batches them.
            # Do not discard later notifications from the same emulation
            # window.  With the broad read watch, the object-name load can
            # follow several ordinary WRAM reads; truncating this batch was
            # capable of deleting the decisive source event from the ledger.
            for params in hook_notes:
                handle = params.get("handle")
                ...
                ledger.append({
                    ...
                    "kind": label,
                    "pc": cpu.get("pc"), "k": cpu.get("k"),
                    "a": cpu.get("a"), "x": cpu.get("x"), "y": cpu.get("y"),
                    "p": cpu.get("p"), "sp": cpu.get("sp"),
                    "address": address,
                    "old": None,
                    "new": value,
                    ...
                    "dst_index": u16(session.read_memory("snesMemory", 0x7E5F9E, 2)),
                    "remaining": u16(session.read_memory("snesMemory", 0x41F6AA, 2)),
                    "remaining_field": u16(session.read_memory("snesMemory", 0x41F6AC, 2)),
                    "source_window": list(session.read_memory("snesMemory", 0x7ECD40, 16)),
                    "controller_buffer": list(session.read_memory("snesMemory", 0x7E5FA4, 32)),
                    "talk_raw": list(session.read_memory("snesMemory", 0x7E7A30, 32)),
                    # Preserve the debugger's event-time fields verbatim;
                    # querying CPU state after a batched notification is not
                    # an atomic attribution of the hooked instruction.
                    "hook_params": dict(params),
                    "cpu_snapshot_after_notification": dict(cpu),
                })
        return {
            "termination": "builder_write_stream_complete",
            "events": ledger,
            "state": {
                "cpu": session.get_cpu_state("Snes"),
                "dst_index": u16(session.read_memory("snesMemory", 0x7E5F9E, 2)),
...
def run_first_a_forensic(session, output: Path, rom_name: str = "") -> dict:
    """Collect one transient first-A trace without a logical-frame hook.

    The input edge is the already-proven one-frame helper.  During the
    transaction, only forensic writer hooks are active; completed-frame
    fencing resumes after release for the final semantic snapshot.
    """
    global LOGICAL_FRAME_HOOK
    # Remove unrelated visual/debug stops.  The transaction must be observed
    # by the forensic set only; a restore or semantic hook firing first would
    # recreate the paused-PC ambiguity this mode is meant to eliminate.
    global RESTORE_DEBUG_HOOK, CONTROLLER_MODE_HOOK, CONTROLLER_TRACE_HOOKS
    for handle in [RESTORE_DEBUG_HOOK, CONTROLLER_MODE_HOOK,
                   *CONTROLLER_TRACE_HOOKS.keys()]:
        if handle is not None:
            session.remove_hook(handle)
    RESTORE_DEBUG_HOOK = None
    CONTROLLER_MODE_HOOK = None
    ...
    # Reproduce the accepted action-edge precondition before arming any
    # forensic stop: one or more completed released frames with HELD,
    # PREVIOUS, and PRESSED all clear.  The earlier builder runs armed the
    # mode hook but omitted this release fence, so they could legitimately
    # miss the A edge even with the cursor at the known point.
    for _ in range(4):
        run_held_level(session, 0, 2)
        # The forensic pass deliberately removes the logical-frame hook; the
        # release fence here is only to clear the input edge before arming the
        # transaction hooks.
        ...
        if not any(baseline):
            break
    if any(baseline):
        raise RuntimeError(f"forensic A baseline was not released: {list(baseline)}")
    hooks: dict[int, str] = {}
    diag_handles = {}
    for value in (0x31, 0x32, 0x33, 0x34):
        handle = session.add_write_hook(0x7E5FEC, match_value=value,
                                        match_value_mask=0xFF)
        diag_handles[value] = handle
        hooks[handle] = f"diag_{value:02x}"
    # Match the success transition specifically.  A generic mode hook can
    # stop later on RefreshSelection's 1->0 clear and falsely make GOOD look
    # like the first-A success.  The zero-valued clear remains armed as a
    # separate forensic hook for the post-success transaction.
    mode_handle = session.add_write_hook(0x7E5FE0, match_value=1,
                                         match_value_mask=0xFF)
    hooks[mode_handle] = "mode_01"
    mode_clear_handle = session.add_write_hook(0x7E5FE0, match_value=0,
                                               match_value_mask=0xFF)
    hooks[mode_clear_handle] = "mode_10_or_reset"
    # These are generated addresses for the two preserved comparison ROMs.
    # They observe the refresh entry/clear target; they do not alter runtime
    # control flow.  The generated map is intentionally used rather than a
    # source-level assumption because the two ROMs have different layouts.
    refresh_addrs = {
        "generic-action-lifecycle3-names9.sfc": (0x99A9, 0x99F8),
        "generic-action-lifecycle4-names26.sfc": (0x99D2, 0x9A21),
    }.get(rom_name)
    if refresh_addrs:
        for address, label in zip(refresh_addrs,
                                  ("refresh_entry", "refresh_clear")):
            handle = session.add_exec_hook(address)
            hooks[handle] = label
    try:
        clear_address = _startup_ns["mapped_cpu_address_for_rom"](
            (ROOT / "build" / rom_name) if rom_name else Path(),
            "ScummV5_Controller_RefreshSelection__clear")
    except Exception:
        clear_address = None
    if clear_address is not None:
        handle = session.add_exec_hook(clear_address)
        hooks[handle] = "refresh_clear_exact"
    # The selected-object validator is only legal after an action has selected
    # an object.  Arm its generated address as an observational hook at the
    # pre-A quiescent boundary so a pre-interaction call cannot be inferred
    # from a later final snapshot.  Older control ROMs do not contain this
    # symbol and simply omit the hook.
    try:
        validate_address = _startup_ns["mapped_cpu_address_for_rom"](
            (ROOT / "build" / rom_name) if rom_name else Path(),
            "ScummV5_Generic_Object_ValidateSelected_Far")
    except Exception:
        validate_address = None
    if validate_address is not None:
        handle = session.add_exec_hook(validate_address)
        hooks[handle] = "validate_selected"
    # Shared generated miss target: the initial A hit succeeds, so any miss
    # observed after mode=1 is attributable to refresh/revalidation.
    miss_handle = session.add_exec_hook(0x98A7)
    hooks[miss_handle] = "object_hittest_miss"
    events: list[dict] = []
    try:
        # The caller established the completed quiescent boundary.  Reconfirm
        # the input level without advancing, then wait on the first expected
        # writer.  All other hooks remain installed during this wait.
        # Use the same one-frame helper that established the accepted R/P/N
        # contract.  The hold=true primitive is reserved for cursor motion;
        # it has not reliably crossed the auto-joypad seam in this forensic
        # runner.
        session.set_input(session.BTN_A, 1)
        # Keep all hooks installed while advancing exactly one emulator frame
        # at a time. This is deliberately not the completed logical-frame
        # hook: it is only the harness' event-loop pacing boundary. The write
        # notifications retain exact frame/cycle/value ordering, and the
        # paused end-of-frame register/memory snapshot is attached to each
        # event without allowing any fired hook to destroy its siblings.
        #
        # The A level is asserted for exactly one frame.  A second frame is
        # allowed only after releasing it, so a later mode write cannot be
        # mistaken for part of the first-A transaction.
        stop = {"reason": "transaction_window", "frames": 0}
        # Use the mode-byte write watch as the stop condition.  Nexen leaves
        # the CPU paused at the actual write, so this snapshot is taken before
        # any later frame can overwrite the selected mode or scratch fields.
        session.pause()
        frame_result = {"framesAdvanced": 1,
                        "notifications": _forensic_notifications(session, hooks)}
        stop["frames"] = int(frame_result.get("framesAdvanced", 0))
        batch = frame_result["notifications"]
        if not any(item.get("kind") == "mode_01" for item in batch):
            ctl = session.read_memory("snesMemory", 0x7E5FE0, 0x0A)
            inp = session.read_memory("snesMemory", 0x7E2200, 6)
            raise RuntimeError(
                f"forensic one-frame A did not reach mode=1: input={list(inp)} "
                f"controller={list(ctl)} notifications={batch}")
        for item in batch:
            event = _forensic_snapshot(session, item["kind"], hook=item,
                                       mode_value=item.get("value"), writer=item["kind"])
            event.update({"observation": "atomic_write_hook_snapshot",
                          "hook_value": item.get("value"),
                          "hook_address": item.get("address"),
                          "input_phase": "press",
                          "event_serial": len(events) + 1})
            events.append(event)
        stop["termination"] = "mode_write"
        # Handoff point: the mode-1 write is the proven selection commit.
        # Keep the CPU in this transaction, release the external A level
        # without advancing emulation, remove the interaction hooks, and arm
        # only the builder observation before resuming.
        set_input_level(session, 0)
        # Validator-only builder window: collect every actual text-buffer,
        # index, and shared-scratch write.  In particular, this distinguishes
        # a zero-filled object copy from an object loop that never stores.
        # The earlier first-A hooks must be removed first: any non-target hook
        # also pauses Nexen, which would prevent the single builder write
        # target below from ever becoming the observed stop.
        for handle in list(hooks):
            try:
                session.remove_hook(handle)
            except Exception:
                pass
        hooks.clear()
        builder_state = _capture_hud_builder_ledger(session, max_events=40)
        builder_state.update({
            "raw_length": session.read_memory("snesMemory", 0x7E7A28, 1)[0],
            "segment_length": session.read_memory("snesMemory", 0x7E7AC3, 1)[0],
            "cpu_at_stop": session.get_cpu_state("Snes"),
            "object_id": u16(session.read_memory("snesMemory", 0x7E5FE6, 2)),
            "object_name_length": session.read_memory(
                "snesMemory", 0x7E8800 + u16(session.read_memory("snesMemory", 0x7E5FE6, 2)), 1)[0],
        })
        stop["termination"] = "selection_to_builder_handoff"
        events.sort(key=lambda item: (int(item.get("cycle", 0)),
                                      int(item.get("event_serial", 0))))
        events.sort(key=lambda item: (int(item.get("cycle", 0)),
                                      0 if item.get("observation") != "hook_notification" else 1))
        set_input_level(session, 0)
        session.resume()
        release = session.run_frames(1)
        session.pause()
        if release.get("timedOut") or release.get("framesAdvanced") != 1:
            raise RuntimeError(f"forensic release failed: {release}")
        # Reinstall the completed-frame hook only for final semantic state.
        LOGICAL_FRAME_HOOK = session.add_exec_hook(LOGICAL_FRAME_ADDRESS)
        advance_logical_safe(session)
        final = read_scene(session)
        report = {"events": events, "mode_stop": stop,
                  "emulator_provenance": EMULATOR_PROVENANCE,
                  "replay_contract": replay_contract_for(Path(rom_name)) if rom_name else None,
                  "hud_builder_window": builder_state,
                  "final_completed_frame": final,
                  "cpu_after": session.get_cpu_state("Snes")}
        output.write_text(json.dumps(report, indent=2) + "\n")
        print("forensic first-A:", json.dumps(report, sort_keys=True), flush=True)
        return report
    finally:
        for handle in list(hooks):
            try:
...
def advance_until(session, predicate, limit: int, description: str) -> dict:
    ...
    def poll():
        ...
        return {
            ...
            "verb": ctl[5],
            "object": u16(ctl[6:]),
            "fixture_request": session.read_memory("snesMemory", 0x7E235E, 1)[0],
            "fixture_active": session.read_memory("snesMemory", 0x7E235F, 1)[0],
            "c17_name_length": session.read_memory("snesMemory", 0x7F6F20 + 3 * 0x60 + 0x18, 1)[0],
            "c17_name": list(session.read_memory("snesMemory", 0x7F6F20 + 3 * 0x60 + 0x20, 8)),
            "object490_state": session.read_memory("snesMemory", OBJECT_STATES + 490, 1)[0],
            "submissions": ctl[9],
            "last_action": ctl[10],
            "talk_active": session.read_memory("snesMemory", 0x7E7A20, 1)[0],
            ...
            "verb_api_object": u16(session.read_memory("snesMemory", 0x7E7EBF, 2)),
            "verb_api_id": u16(session.read_memory("snesMemory", 0x7E7EC1, 2)),
            "verb_api_result": u16(session.read_memory("snesMemory", 0x7E7EC3, 2)),
            "verb_cycle_after": u16(session.read_memory("snesMemory", 0x7E5F9C, 2)),
            "verb_cycle_limit": u16(session.read_memory("snesMemory", 0x7E5F98, 2)),
            "active_record": session.read_memory("snesMemory", 0x7FF2BE, 1)[0],
    ...
    if CONTROLLER_MODE_HOOK is not None or CONTROLLER_TRACE_HOOKS:
        notifications = session.drain_notifications(timeout=0.2)
        state["controller_mode_writes"] = [
            item for item in notifications
            if item.get("params", {}).get("handle") == CONTROLLER_MODE_HOOK
        ]
        state["controller_trace_hooks"] = [
            {**item, "label": CONTROLLER_TRACE_HOOKS.get(
                item.get("params", {}).get("handle"))}
            for item in notifications
            if item.get("params", {}).get("handle") in CONTROLLER_TRACE_HOOKS
        ]
...
def wait_for_semantic_moving_snapshot(session, hook_handle: int,
    ...
    for _ in range(limit):
        result = session.run_until(max_frames=600, hook_handle=hook_handle)
        if int(result.get("framesAdvanced", 0)) <= 0:
            # Nexen reports a write watch before the 65816 store is visible.
            # Fence one complete logical frame, then re-arm the same semantic
            # field watch instead of treating the pre-write pause as a state
            # snapshot.
            session.remove_hook(hook_handle)
            session.resume()
            follow = session.run_frames(1)
            session.pause()
            raw = bytes(session.read_memory("snesMemory", 0x7E5E42, 0x0C))
            if raw[0x04] == 1 and raw[0x0B]:
                return {
                    "x": u16(raw, 0x00),
                    "y": u16(raw, 0x02),
                    "select": raw[0x04],
                    ...
                    "costume": raw[0x0A],
                    "visible": raw[0x0B],
                    "frames_advanced": int(follow.get("framesAdvanced", 0)),
                }
            hook_handle = session.add_write_hook(
                0x7E5E46, match_value=1, match_value_mask=0xFF)
            continue
        raw = bytes(session.read_memory("snesMemory", 0x7E5E42, 0x0C))
        last = {
            "x": u16(raw, 0x00),
...
def wait_for_committed_generation(session, hook_handle: int,
                                  generation: int, limit: int = 128) -> dict:
    ...
    for _ in range(limit):
        ...
        # Nexen write hooks stop before the CPU store becomes observable.  A
        # hook-fired result with zero frames is therefore a notification of a
        # candidate commit, not a coherent post-write snapshot.  Let this
        # logical frame finish once, then inspect the committed generation.
        post_write_fence = False
        if (result.get("reason") == "hookFired"
                and int(result.get("framesAdvanced", 0)) == 0):
            post_write_fence = True
            # Keep the hook from immediately stopping the completion frame on
            # the same pre-write notification.  Re-arm it after the coherent
            # frame boundary and return the replacement handle to the caller.
            session.remove_hook(hook_handle)
            session.resume()
            follow = session.run_frames(1)
            session.pause()
            hook_handle = session.add_write_hook(0x401010, 0x401011)
            result = {
                **result,
                "post_write_frames": int(follow.get("framesAdvanced", 0)),
                "post_write_timed_out": bool(follow.get("timedOut", False)),
            }
        ...
        if committed == generation:
            return {
                ...
                "frames_advanced": int(result.get("framesAdvanced", 0)),
                "_hook_handle": hook_handle,
            }
        if len(windows) < 8:
            progress = read_service_progress(session)
            windows.append({
                "reason": result.get("reason"),
                "frames_advanced": int(result.get("framesAdvanced", 0)),
                "post_write_fence": post_write_fence,
                "post_write_frames": int(result.get("post_write_frames", 0)),
                "backend_steps": progress["backend_steps"],
                "state": backend[0x13],
                "locked": backend[0x14],
                "pending_generation": u16(backend, 0x0E),
                "committed_generation": committed,
                "candidates": u16(backend, 0x22),
                "pending_tiles": u16(backend, 0x24),
                "inflight": u16(backend, 0x26),
                "converted": u16(backend, 0x2A),
            })
        effective_frames = int(result.get("framesAdvanced", 0))
        effective_frames += int(result.get("post_write_frames", 0))
...
def read_surface_quiescent(session) -> dict:
    """Read the complete committed-surface readiness contract."""
    scene = read_scene(session)
    surface = bytes(session.read_memory("snesMemory", 0x401080, 0x40))
    backend = bytes(session.read_memory("snesMemory", 0x401000, 0x40))
    fifo = read_event_fifo(session)
    surface_generation = u16(surface, 0x02)
    next_generation = u16(surface, 0x1C)
    presented_room = surface[0x20]
    # No production writer currently updates this field.  Retain it only as
    # stale/reserved diagnostic bookkeeping; it is not readiness evidence.
    presented_generation_stale = u16(surface, 0x22)
    live_surface_packets = [
        packet for packet in fifo["packets"]
        if packet["service"] == 1 and packet["opcode"] in (0x0C, 0x11)
    ]
    return {
        "room": scene["room"],
        "phase": scene["phase"],
        "error": scene["error"],
        "talk": scene["talk_active"],
        "c20": scene["c20"],
        "sentence_pending": scene["sentence_api_pending"],
        "pending_visual": surface[0x26],
        "pending_room": surface[0x27],
        "presented_room": presented_room,
        "surface_generation": surface_generation,
        "next_generation": next_generation,
        "presented_generation_stale": presented_generation_stale,
        "backend_state": backend[0x13],
        "surface_locked": backend[0x14],
        "backend_pending_generation": u16(backend, 0x0E),
        "backend_committed_generation": u16(backend, 0x10),
        "fifo": fifo,
        "live_surface_packets": live_surface_packets,
    }


def read_surface_progress_sample(session, elapsed: int) -> dict:
    """Capture bounded conversion progress at a completed-frame boundary."""
    surface = bytes(session.read_memory("snesMemory", 0x401080, 0x40))
    backend = bytes(session.read_memory("snesMemory", 0x401000, 0x40))
    dma = bytes(session.read_memory("snesMemory", 0x7E223A, 0x10))
    progress = read_service_progress(session)
    ...
    return {
        "elapsed_completed_frames": elapsed,
        "frame": progress["frame_counter"],
        "logical_frame_count": progress["logical_frame_count"],
        "backend_steps": progress["backend_steps"],
        "backend_stage": progress["backend_stage"],
        "backend_state": backend[0x13],
        "surface_locked": backend[0x14],
        "next_generation": u16(surface, 0x1C),
        "backend_pending_generation": u16(backend, 0x0E),
        "backend_committed_generation": u16(backend, 0x10),
        "candidate_tiles": u16(backend, 0x22),
        "pending_tiles": u16(backend, 0x24),
        "inflight_tiles": u16(backend, 0x26),
        "expected_dma": u16(backend, 0x28),
        "converted_tiles": u16(backend, 0x2A),
        "scan_tile": u16(backend, 0x38),
        "scan_palette": u16(backend, 0x3A),
        "dma_words": [word(dma, 0), word(dma, 2), word(dma, 4), word(dma, 6)],
        "accepted_present": progress["accepted_present"],
        "rejected_present": progress["rejected_present"],
    }


def wait_for_surface_quiescent(session, limit: int = 512) -> dict:
    """Fence a stable room presentation at completed logical-frame boundaries."""
    last = None
    samples = []
    sample_points = {0, 1, 8, 16, 32, 64, 128, 256, 512}
    elapsed = 0
    no_progress = 0
    last_progress = None
    stall_limit = 64
    while elapsed <= limit:
        state = read_surface_quiescent(session)
        last = state
        if elapsed in sample_points:
            sample = read_surface_progress_sample(session, elapsed)
            samples.append(sample)
            print("surface fence progress:", json.dumps(sample, sort_keys=True), flush=True)
        progress = read_surface_progress_sample(session, elapsed)
        progress_key = tuple(progress[key] for key in (
            "backend_steps", "candidate_tiles", "pending_tiles", "inflight_tiles",
            "converted_tiles", "scan_tile", "scan_palette",
            "backend_pending_generation", "backend_committed_generation",
            "backend_state", "accepted_present"))
        ...
        if no_progress >= stall_limit:
            raise RuntimeError(
                "surface presentation made no measurable progress at completed "
                f"logical-frame boundaries for {stall_limit} frames: "
                f"elapsed={elapsed} state={state} progress={progress} "
                f"samples={samples} cpu={session.get_cpu_state('Snes')}"
            )
        ready = (
            state["room"] == 42 and state["phase"] == 0 and state["error"] == 0
            and state["talk"] == 0 and state["c20"] == 0
            and state["sentence_pending"] == 0
            and state["pending_visual"] == 0
            and state["presented_room"] == 42
            and state["backend_state"] == 1
            and state["surface_locked"] == 0
            and state["backend_pending_generation"] == state["next_generation"]
            and state["backend_committed_generation"] == state["next_generation"]
            and not state["live_surface_packets"]
        )
        if ready:
            generation = state["next_generation"]
            advance_logical_safe(session)
            elapsed += 1
            stable = read_surface_quiescent(session)
            if (stable["next_generation"] == generation
                    and stable["backend_committed_generation"] == generation
                    and stable["backend_state"] == 1
                    and stable["surface_locked"] == 0
                    and stable["backend_pending_generation"] == generation
                    and stable["pending_visual"] == 0
                    and not stable["live_surface_packets"]):
                stable["progress_samples"] = samples
                return stable
        advance_logical_safe(session)
        elapsed += 1
    progress = read_service_progress(session)
    raise RuntimeError(
        f"surface did not reach stable quiescence: {last}; "
        f"service_progress={progress}; samples={samples}; "
        f"cpu={session.get_cpu_state('Snes')}")


def wait_for_overlay_committed(session, limit: int = 256) -> dict:
    """Fence the selected text overlay without changing production timing."""
    for _ in range(limit):
        raw = session.read_memory("snesMemory", 0x41F614, 0x20)
        state = raw[0]
        current = int.from_bytes(raw[2:4], "little")
        committed = int.from_bytes(raw[6:8], "little")
        next_generation = int.from_bytes(raw[0x18:0x1A], "little")
        if current != 0 and current == committed and committed == next_generation:
            return {"state": state, "current": current, "committed": committed,
                    "next": next_generation}
        advance_logical_safe(session)
    raise RuntimeError(f"text overlay did not commit: state={state} current={current} "
...
def main() -> int:
    global HUD_TRACE_HOOKS, EMULATOR_PROVENANCE
    ...
    parser.add_argument("--skip-visual-ready", action="store_true",
                        help="semantic-only diagnostic: do not require the room PRESENT gate")
    parser.add_argument("--logical-frame-fence", action="store_true",
                        help="deprecated; completed logical-frame fencing is always enabled")
    ...
    EMULATOR_PROVENANCE = emulator_provenance_for(args.nexen)
    ...
    with mcp_session.McpSession(
        ...
        for frame in range(1, args.startup_frames):
            ...
            # Phase 2 is the room-entry/ENCD execution phase, not an
            # interactive-ready boundary.  Require the completed phase-0
            # lifecycle and an idle engine frame before installing the
            # presentation fence; otherwise the first fence advance can
            # observe the middle of ENCD and manufacture a visual stall.
            engine_busy = post.get("engine_busy", session.read_memory("snesMemory", 0x7E2231, 1)[0])
            if (post["room"] == 42 and post["phase"] == 0 and engine_busy == 0
                    and post["error"] == 0
        ...
        # From this accepted startup boundary onward, all semantic and
        # presentation predicates use the completed logical-frame fence.
        # Startup itself uses one completed emulator frame per title edge;
        # installing the execution hook earlier interferes with the MCP input
        ...
        if args.skip_visual_ready:
            ...
            # frames after the room boundary so the controller can publish
            # its initial source-driven selection.  This does not assert or
            # bypass presentation readiness; it only avoids sampling the
            # controller during room-install publication.
            advance_logical_count(session, 8)
        ...
        visual_ready = False
        surface_state = session.read_memory("snesMemory", 0x401080, 0x34)
        for visual_poll in range(0 if args.skip_visual_ready else 300):
            surface_state = session.read_memory("snesMemory", 0x401080, 0x34)
            backend = session.read_memory("snesMemory", 0x401000, 0x40)
            event_state = session.read_memory("snesMemory", 0x7E2000, 0x06)
            accepted_present = int.from_bytes(backend[0x1A:0x1C], "little")
            ...
            if (surface_state[0] == 42 and surface_state[4] == 1
                ...
                visual_ready = True
                break
            if visual_poll in (0, 1, 8, 16, 32, 64, 128, 256, 299):
                print("visual readiness progress:", json.dumps({
                    "poll": visual_poll,
                    "room": surface_state[0],
                    "surface_status": surface_state[4],
                    "pending_visual": surface_state[0x26],
                    "accepted_present": accepted_present,
                    "event_count": event_count,
                    "backend": list(backend[:0x2C]),
                    "service_progress": read_service_progress(session),
                    "scene": read_scene(session),
                    "cpu": session.get_cpu_state("Snes"),
                    "reset_diag": list(session.read_memory("snesMemory", 0x7E1020, 0x20)),
        ...
        if not visual_ready and not args.skip_visual_ready:
            require(False, f"room-42 visual publication was not reached; state={list(surface_state)}")
        if not visual_ready:
            events.append({"stage": "visual_ready_skipped_semantic_only",
                           "state": read_scene(session),
                           "surface_state": list(surface_state)})
        events.append({"stage": "visual_ready", "state": read_scene(session)})
        print("native stage: ready checkpoint", flush=True)
        if args.skip_visual_ready:
            quiescent = {"skipped": True, "reason": "visual_ready_skipped_semantic_only"}
            events.append({"stage": "surface_quiescent_skipped", "state": quiescent})
            print("semantic stage: surface quiescence skipped", read_scene(session),
                  flush=True)
        else:
            quiescent = wait_for_surface_quiescent(session)
            events.append({"stage": "surface_quiescent", "state": quiescent})
            print("native stage: ready settled", read_scene(session),
                  "surface_quiescent", quiescent, flush=True)
        ...
        if args.capture_only:
            ...
            (args.output / "report.json").write_text(json.dumps({
                ...
                "replay_contract": replay_contract_for(args.rom),
        ...
        for _ in range(128):
            ...
            if locker_left <= room_x < locker_right and locker_top <= room_y < locker_bottom:
                break
            if room_x < locker_left:
                button = session.BTN_RIGHT
            elif room_x >= locker_right:
                button = session.BTN_LEFT
            elif room_y < locker_top:
                button = session.BTN_DOWN
            else:
                button = session.BTN_UP
            before_x = cursor_x
        ...
        # The first authored verb is already selected by the generic verb
        # discovery path.  Submit it unchanged for the Open regression;
        # Y is reserved for the later state-neutral branch after this action
        # has released the generic lifecycle.
        first_verb = read_scene(session)
        require(first_verb["object"] == 490 and first_verb["verb"] == 3,
                f"source-first locker verb was not selected: {first_verb}")
        # C17 verb records may be installed by an authored startup child
        # after the generic source hit/verb selection has already succeeded.
        # Keep the semantic selection, but do not submit or capture a HUD
        # frame until the selected runtime record has usable text.
        ...
        # C17 is the semantic source of the selected verb name.  The
        # controller-text valid bit is a transient service handoff: the
        # overlay consumer clears it after copying the text, so polling it at
        # a later completed frame is not a publication proof.  Native HUD
        # publication is fenced below by the overlay commit and capture.
        events.append({"stage": "verb_selected", "state": first_verb})
        if not args.skip_visual_ready:
            open_hud_quiescent = wait_for_surface_quiescent(session)
            events.append({"stage": "open_hud_surface_quiescent",
                           "state": open_hud_quiescent})
            try:
                overlay_commit = wait_for_overlay_committed(session)
            except RuntimeError as exc:
                trace_notifications = session.drain_notifications(timeout=0.2)
                overlay_commit = {"status": "not_observed", "error": str(exc),
                    "hud_trace": [
                        {**item, "label": HUD_TRACE_HOOKS.get(
                            item.get("params", {}).get("handle"))}
                        for item in trace_notifications
                        if item.get("params", {}).get("handle") in HUD_TRACE_HOOKS
                    ], "scene": read_scene(session)}
            events.append({"stage": "open_hud_overlay_committed",
                           "state": overlay_commit})
            capture_native(session, args.output / "02-open-selected.png")
            # HUD trace hooks are diagnostic only.  They pause the emulator
            # at service entry; remove them before the next controller edge
            # so later input is fenced only at completed logical frames.
            for handle in list(HUD_TRACE_HOOKS):
                session.remove_hook(handle)
            HUD_TRACE_HOOKS = {}
        ...
        if not args.skip_visual_ready and read_scene(session)["mode3_state"] != 1:
        ...
        global SEMANTIC_MOVING_HOOK
        semantic_moving_hook = session.add_write_hook(
            0x7E5E46, match_value=1, match_value_mask=0xFF)
        SEMANTIC_MOVING_HOOK = semantic_moving_hook
        global CONTROLLER_MODE_HOOK
        CONTROLLER_MODE_HOOK = session.add_write_hook(0x7E5FE0)
        global CONTROLLER_TRACE_HOOKS
        CONTROLLER_TRACE_HOOKS = {
            session.add_exec_hook(0x9968): "action_pending",
            session.add_exec_hook(0x99A9): "refresh_selection",
            session.add_exec_hook(0x983F): "cursor_a_hit_test",
            session.add_exec_hook(0x9882): "cursor_hit_ok",
            session.add_exec_hook(0x98BD): "cursor_verb_ok",
        }
        action_tap(session, session.BTN_A)
        print("after Open submit edge:", read_scene(session), flush=True)
        ...
        backend_commit_hook = walking_commit.pop("_hook_handle", backend_commit_hook)
        ...
        # Keep the state-neutral authored action in the same room/session. The
        # generic controller revalidates the source object/verb set after the
        # Open sentence releases; it does not infer an Inspect action from
        # object state. Cycle the authored verb through the normal Y input.
        advance_logical_safe(session)
        capture_native(session, args.output / "03-opened.png", wait_for_quiet=False)
        require(read_scene(session)["room"] == 42,
                "room changed before the next generic action")
        action_ready = advance_until(
            session, lambda s: s["mode"] == 1 and not s["talk_active"]
            and s["c20"] == 0 and s["sentence_api_pending"] == 0,
            600, "generic action completion/reselection")
        events.append({"stage": "action_released", "state": action_ready})
        verb_before_neutral = action_ready["verb"]
        action_tap(session, session.BTN_Y)
        neutral_ready = advance_until(
            session, lambda s: s["mode"] == 1 and s["verb"] != verb_before_neutral,
            64, "next authored verb selection")
        require(neutral_ready["object"] == 490,
                f"revalidated object was not retained by source hit test: {neutral_ready}")
        events.append({"stage": "state_neutral_verb_selected", "state": neutral_ready})
        action_tap(session, session.BTN_A)
        events.append({"stage": "state_neutral_submitted", "state": read_scene(session)})
        print("native stage: state-neutral authored action submitted", events[-1]["state"], flush=True)
        print("state-neutral sentence diagnostics", {
            "cpu": session.get_cpu_state("Snes"),
            "reset_diag": list(session.read_memory("snesMemory", 0x7E1020, 0x20)),
            "events": list(session.read_memory("snesMemory", 0x7E2000, 0x06)),
        ...
        try:
            ...
            (args.output / "state-neutral-boundary-trace.json").write_text(
                json.dumps(trace, indent=2) + "\n")
        except Exception as exc:
            print("state-neutral boundary trace unavailable:", exc, flush=True)
        # This is deliberately a generic lifecycle assertion.  The selected
        # authored verb is state-neutral and need not start a talk.  Completion
        # is established by the canonical sentence/C20/movement boundary, not
        # by an object-state transition or an assumed dialogue side effect.
        neutral_complete = advance_until(
            session,
            lambda s: s["mode"] == 1 and not s["talk_active"]
            and s["c20"] == 0 and s["sentence_api_pending"] == 0
            and s["moving1"] == 0 and s["error"] == 0,
            600,
            "state-neutral action completion/reselection",
        )
        require(neutral_complete["object"] == 490,
                f"state-neutral action lost the selected object: {neutral_complete}")
        require(neutral_complete["verb"] != 0,
                f"state-neutral action did not refresh authored verbs: {neutral_complete}")
        require(neutral_complete["object490_state"] == 1,
                "state-neutral action did not preserve the Open effect")
        events.append({"stage": "state_neutral_complete", "state": neutral_complete})
        capture_native(session, args.output / "04-state-neutral-complete.png")
        ...
        # Bind a native HUD capture to the completed source-derived selection
        # boundary.  This is separate from the semantic object assertion so
        # a black or stale screenshot cannot masquerade as C17 evidence.
        capture_native(session, args.output / "06-object492-selected.png")
        ...
        advance_logical_count(session, 2)
        post_dialogue = read_scene(session)
        if post_dialogue["cursor_x"] <= cursor_before:
            # The source-derived object-492 point can leave the cursor at the
            # right edge of its reachable range.  Prove re-entry with the
            # opposite ordinary controller edge rather than mistaking a
            # clamped rightward move for a dead controller.
            tap(session, session.BTN_LEFT)
            advance_logical_count(session, 2)
            post_dialogue = read_scene(session)
        ...
        # The preceding source-derived A selection of object 492 is the
        # authoritative post-action input witness.  At this point the cursor
        # may be clamped by the selected object's reachable point, so a
        # directional displacement is not a valid lifecycle requirement.
        require(post_dialogue["mode"] in (0, 1) and post_dialogue["error"] == 0,
                "controller did not remain in usable selection mode")
        advance_logical_safe(session)
