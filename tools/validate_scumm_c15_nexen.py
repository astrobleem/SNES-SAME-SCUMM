#!/usr/bin/env python3
"""Validate C15 SCUMM v5 actor-follow camera intent in Nexen."""
...
def snapshot(session: object) -> dict[str, object]:
    ...
    follow = session.read_memory("snesMemory", FOLLOW_ADDRESS, 4)
    return {
        ...
        "camera_follow_actor": None if follow[0] == 0xFF else follow[0],
        "camera_mode": follow[1], "moving_to_actor": bool(follow[2]),
        "follow_state_initialized": bool(follow[3]),
    }
...
def main() -> int:
    ...
    output = (args.output or ROOT / "build" / f"scumm-c15-nexen-{rom_hash[:16]}").resolve()
    output.mkdir(parents=True, exist_ok=True)
    report_path = output / "report.json"
    report: dict[str, object] = {
        "gate": "C15-actor-follow-camera", "result": "running",
    ...
    try:
        with mcp_session.McpSession(
            ...
            for _ in range(3):
                step_video_frame(session)
                first = snapshot(session)
                if first["fixture_active"] == FIXTURE_ID and first["pc"] == 8:
                    break
            first["video_frame"] = session.get_state()["frameCount"]
            report["first_tick"] = first
            expected_first = {
                "pc": 8, "status": 2, "error": 0, "last_opcode": 0x80,
                "frame_count": 1, "frame_ops": 3, "total_ops": 3,
                "fixture_active": FIXTURE_ID, "program_select": FIXTURE_ID,
                "camera_follow_actor": 3, "camera_mode": 1,
                "moving_to_actor": False, "follow_state_initialized": True,
            }
            ...
            expected_terminal = {
                "pc": 12, "status": 2, "error": 0, "last_opcode": 0x80,
                "frame_count": 2, "frame_ops": 2, "total_ops": 5,
                "fixture_active": FIXTURE_ID, "program_select": FIXTURE_ID,
                "camera_follow_actor": 7, "camera_mode": 1,
                "moving_to_actor": False, "follow_state_initialized": True,
            }
        ...
        print(f"C15 SCUMM actorFollowCamera: FAIL: {exc}", file=sys.stderr)
        print(report_path)
        return 1
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print("C15 SCUMM actorFollowCamera: PASS (direct/variable follow intent and yield persistence)")
    print(report_path)
    return 0
