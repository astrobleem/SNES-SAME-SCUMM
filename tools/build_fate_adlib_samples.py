#!/usr/bin/env python3
"""Build loop candidates from explicitly supplied external sound captures."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--patch-records", type=Path, required=True,
    )
    parser.add_argument(
        "--output", type=Path,
        default=ROOT / "build/external-audio/fate154",
        help="generated output directory (defaults outside tracked source)",
    )
    args = parser.parse_args()
    output_dir = args.output.resolve()
    if output_dir.is_relative_to(ROOT) and not output_dir.is_relative_to(ROOT / "build"):
        parser.error("generated media output inside the repository must stay under build/")
    captures = {
    ...
    for name, patch, note_index, frequency, first_octave, last_octave in JOBS:
        ...
        records.append({
            ...
            "sample_resource": os.path.relpath(path.resolve(), ROOT),
    ...
    manifest = output_dir / "fate154_adlib_manifest.json"
    bank = {
