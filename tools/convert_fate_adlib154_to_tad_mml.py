...
def load_instrument_bank(path: Path) -> InstrumentBank:
    bank = InstrumentBank.decode(path.read_bytes(), str(path))
...
def collect(
    ...
    if bank is None:
        raise ValueError("an explicit, externally supplied instrument bank is required")
...
def render(sound: ScummV5EmbeddedSound, bank: InstrumentBank) -> str:
...
def main() -> int:
    ...
    parser.add_argument("--bank", type=Path, required=True,
                         help="externally supplied instrument-zone bank")
    ...
    bank = load_instrument_bank(args.bank)
    text = render(sound, bank)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(text, encoding="utf-8", newline="\n")
    notes = collect(sound, bank=bank)
