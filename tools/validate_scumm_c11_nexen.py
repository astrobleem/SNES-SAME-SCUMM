#!/usr/bin/env python3
"""Validate C11 SCUMM v5 deterministic getRandomNr in Nexen."""
...
def snapshot(session: object) -> dict[str, object]:
    ...
    local0 = session.read_memory("snesMemory", LOCAL_ADDRESS, 2)
    random_state = session.read_memory("snesMemory", RANDOM_STATE_ADDRESS, 2)
    return {
        ...
        "variables": [u16(common, 0x20 + index * 2) for index in range(5)],
        "fixture_active": common[0x5F], "program_select": common[0x62],
        "local0": u16(local0), "random_state": u16(random_state),
    }
...
def main() -> int:
    ...
    parser.add_argument("--port", type=int, default=43996)
    ...
    output = (args.output or ROOT / "build" / f"scumm-c11-nexen-{rom_hash[:16]}").resolve()
    output.mkdir(parents=True, exist_ok=True)
    report_path = output / "report.json"
    report: dict[str, object] = {
        "gate": "C11-deterministic-random", "result": "running",
        ...
        "algorithm": "galois-lfsr16-b400-high-byte-mod-inclusive",
        "seed": 0xACE1,
    }
    ...
    try:
        with mcp_session.McpSession(
            ...
            expected = {
                "pc": 28, "status": 2, "error": 0, "last_opcode": 0x80,
                "frame_count": 1, "frame_ops": 7, "total_ops": 7,
                "variables": [9, 6, 3, 0, 28],
                "fixture_active": FIXTURE_ID, "program_select": FIXTURE_ID,
                "local0": 4, "random_state": 0x0E27,
            }
        ...
        print(f"C11 SCUMM getRandomNr: FAIL: {exc}", file=sys.stderr)
        print(report_path)
        return 1
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print("C11 SCUMM getRandomNr: PASS (inclusive bounds and deterministic state)")
    print(report_path)
    return 0
